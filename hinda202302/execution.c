...
/*   Updated: 2023/02/20 13:29:47 by imraoui          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

void	exec_cmd(t_cmd *exec, t_data *mini, t_token *list)
{
	if (exec->nbr_cmd == 1)
	{
...
void	fork_pipe(t_cmd *pipex, t_data *mini)
{
	...
	if (pipex -> pid[pipex -> i] == 0)
	{
		...
		child(pipex, mini->env, mini);
	}
}

void	exec_pipe(t_cmd *pipex, t_token *list, t_data *mini)
{
	parse_cmd_table(list, &pipex);
